...
import { useMindMapTool } from '../services/api';
import { FiMap, FiLoader, FiAlertTriangle } from 'react-icons/fi';

const MindMapCreator = ({ onSubmit, result, loading, error: parentError }) => {
  ...
  useEffect(() => {
    if (result) {
      console.log('MindMapCreator nhận kết quả:', result);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onSubmit) {
        ...
        const response = await useMindMapTool(topic);
        setMindMap(response.response);
      }
    } catch (err) {
      console.error('Error creating mind map:', err);
      setError('Đã xảy ra lỗi khi tạo sơ đồ tư duy. Vui lòng thử lại sau.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="h-full flex flex-col">
      <div className="bg-gray-800 rounded-lg p-6 mb-6">
        ...
        <p className="text-gray-300 mb-4">
          Nhập chủ đề bạn muốn tạo sơ đồ tư duy và hệ thống sẽ tạo sơ đồ dựa trên tài liệu có sẵn.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          <div className="flex flex-col md:flex-row gap-4">
            <input
              ...
              placeholder="Nhập chủ đề cần tạo sơ đồ tư duy (ví dụ: Machine Learning, Neural Networks, etc.)"
